...
// $image_path	=	substr($_GET['image'], strpos($_GET['image'],'uploads/')+8 , strlen($_GET['image']));	//	 remove the path before uploads/ from image name
$image_path	=	str_replace('http://' . $_SERVER['HTTP_HOST'] . '/' , '' , $_GET['image']);

$_GET['image']	=	'/'.$image_path;
...
$image_array= explode("/",$_GET['image']);

$image      = $image_array[count($image_array)-1];
$image_name = explode(".",$image);
$image_name[1] = $image_name[count($image_name)-1];

$sub_dir1 = substr($image_name[0],0,1);
$sub_dir2 = substr($image_name[0],0,2);
$sub_dir3 = substr($image_name[0],0,4);
		

$img_width	=	$_GET['width'];
if(isset($_GET['height'])){
	$img_width	=	$_GET['width'].'x'.$_GET['height'];
}

$main_directory	=	CURRENT_DIR . CACHE_DIR_NAME;
if(!is_dir($main_directory)){
	mkdir($main_directory, 0777);
}

if(!is_dir($main_directory.'/'.$img_width)){
	mkdir($main_directory.'/'.$img_width, 0777);
}
if(!is_dir($main_directory.'/'.$img_width.'/'.$sub_dir1)){
	mkdir($main_directory.'/'.$img_width.'/'.$sub_dir1, 0777);
}
if(!is_dir($main_directory.'/'.$img_width.'/'.$sub_dir1.'/'.$sub_dir2)){
	mkdir($main_directory.'/'.$img_width.'/'.$sub_dir1.'/'.$sub_dir2, 0777);
}
if(!is_dir($main_directory.'/'.$img_width.'/'.$sub_dir1.'/'.$sub_dir2.'/'.$sub_dir3)){
	mkdir($main_directory.'/'.$img_width.'/'.$sub_dir1.'/'.$sub_dir2.'/'.$sub_dir3, 0777);
}


$folder_name 	= $img_width.'/'.$sub_dir1.'/'.$sub_dir2.'/'.$sub_dir3.'/';


define('CACHE_DIR',				CURRENT_DIR . CACHE_DIR_NAME.$folder_name);
