...
<div id="pagepiling">
<div class="section cms-wrapper mobile-view-wrapper" id="section1">
  <div class="container">
	<div class="row">
    	<div class="col-sm-4 col-md-3 col-lg-3 search-box-wrapper">
			<div class="search-box">
				<div class="form-group">
					<input name="keyword" data-url='{{route("Library.articleDetail","$topicData->slug")}}' type="text" class="form-control search-bar valid" placeholder="Search" value="<?php if(isset($keyword) && !empty($keyword)){ echo $keyword; }?>"/>
					<span><i class="fa fa-search" aria-hidden="true"></i></span>
				</div>
			</div>
		<div class="clearfix"></div>
    	</div>
    	<div class="col-sm-8 col-md-9 col-lg-9 action-button-wraper">
    		<div class="action_button" id="action_button">
				
				<?php /*<a title="Print" onclick="javascript:printContent('blog-detail');" id="mail_button" class="btn btn-default btn-small mail_button"><i class="fa fa-print"></i></a>&nbsp;&nbsp; */?>
   ...
   <div class="clearfix"></div>
    <div class="row">
        ...
        <div class="boder-right" id="blog-detail">
			...
			<div class="folder-decription">
				<h5 class="blog-slug">
					@if(!empty($topicData->user_id)) @if($topicData->user_id!==1){{'By: '.ucfirst($topicData->username)}}@else{!!trans("<b>Posted By</b>: Administrator")!!}@endif @endif <span class="px-10"></span><span class="text-yellow">
					{!! '<b>On</b>: '.date("F m, Y",strtotime($topicData->created_at)) !!}</span>
				</h5>
				<div class="folder-main-sec">
					@if($topicData->image != '' && File::exists(PROJECT_ARTICLE_IMAGE_ROOT_PATH.$topicData->image))
						<figure class="floder-img">
							<img  src="<?php echo PROJECT_ARTICLE_IMAGE_URL.'/'.$topicData->image ?>">
						</figure>
					@endif
					<div class="description">
						{!! $topicData->article_description !!}
					</div>
				</div>
			</div>
		</div>
		<div class="leave_a_comment_box">
			<div class="blog-bottom">
				<span>
				<i class="fa fa-comment blog-icon-bottom"></i>
					<?php echo $topicData->total_comments; ?>
				</span>
				<span>
					<i class="fa fa-eye blog-icon-bottom"></i>
					<?php echo $topicData->viewed; ?>	
				</span>
			</div>
		</div>
		<div class="col-sm-6 col-md-6 col-lg-6">
              <div class="share-this"> 
				...
				?>
                <a title="Facebook" onclick="window.open('<?php echo $facebook_share_link; ?>','name','width=600,height=400')"><i class="fa fa-facebook"></i></a> 
                <a title="Twitter" onclick="window.open('<?php echo $twitter_share_link; ?>','name','width=600,height=400')"><i class="fa fa-twitter"></i></a> 
                <a title="Google Plus" onclick="window.open('<?php echo $google_share_link; ?>','name','width=600,height=400')"><i class="fa fa-google-plus"></i></a>
                <a title="Mail" href="articledetail=<?php echo $articleName; ?><?php echo "__@@__"; ?><?php echo Str::limit(strip_tags($articleDesc),400); ?>" id="print_button"><i class="fa fa-envelope"></i></a> </div>
        </div>
		<!-- MObile Sidebar -->
          <div class="clearfix"></div>
			   ...
			   {{Form::hidden('article_id',$topicData->id)}}
				 <div class="comment-form clearfix">
				   <div class="section-field textarea">
						{{ Form::textarea("description",'', ['class' => 'form-control input-message valid autoExpand','id' => 'description','placeholder'=>'Comment*','rows'=>2,'data-min-rows'=>2]) }}
						<span class="help-inline" id="comment_description_error"></span>
					</div>
					<a class="button pull-left mt-20" href="javascript:void(0);" onclick="saveComment();">
	...
	@foreach($comments as $key=>$articleComment)
		 <div class="comments-1">
			<div class="comments-photo">
			   <figure> 
					@if($articleComment->user_image != '' && File::exists(USER_PROFILE_IMAGE_ROOT_PATH.$articleComment->user_image))
						<img class="img-responsive" src="<?php echo USER_PROFILE_IMAGE_URL.'/'.$articleComment->user_image ?>">
					@else
						<img class="img-responsive" src="<?php echo WEBSITE_IMG_URL ?>admin/no_image.jpg">
					@endif
				</figure>
			</div>
			<div class="comments-info">
			   <h4 class="text-blue"> {{$articleComment->user_name}} <span>
				   {{ date(Config::get("Reading.date_time_format"),strtotime($articleComment->created_at)) }}</span></h4>
				@if(($topicData->allow_comments == 1) && ($topicData->comment_end_date != '0000-00-00') && ($topicData->comment_end_date." 23:59:59" > date("Y-m-d H:i:s")))
				<div class="port-post-social pull-right">
				  <a href="javascript:void(0);" onclick='toggleOn("reply{{$key}}")'>Reply</a>
				</div>
				@endif
		...
		@if(!empty($user_id))
			...
			{{Form::hidden('comment_id',$articleComment->id)}}
				<h3 class="text-blue mb-30">Leave a Reply <a href="javascript:void(0);" onclick='toggleOff("reply{{$key}}")' class="cancel-reply">Cancel Reply</a></h3>
				 <div class="comment-form clearfix">
					<div class="section-field textarea">
						{{ Form::textarea("reply",'', ['class' => 'form-control input-message valid autoExpand','id' => 'reply','placeholder'=>'Reply*','rows'=>2,'data-min-rows'=>2]) }}
						<span class="help-inline" id="reply_description_error"></span>
					</div>
					<a class="button pull-left mt-20" href="javascript:void(0);" onclick="saveCommentReply({{$key}});">
		...
		@if(!empty($articleComment->reply))
			@foreach($articleComment->reply as $k=>$reply)
			 ...
			 <div class="comments-photo">
				<figure>
					@if($reply->user_image != '' && File::exists(USER_PROFILE_IMAGE_ROOT_PATH.$reply->user_image))
						<img class="img-responsive" src="<?php echo USER_PROFILE_IMAGE_URL.'/'.$reply->user_image ?>">
					@else
						<img class="img-responsive" src="<?php echo WEBSITE_IMG_URL ?>admin/no_image.jpg">
					@endif
				</figure>
			   ...
			   <div class="comments-info">
					<h4 class="text-blue">{{$reply->user_name}} <span>
					{{ date(Config::get("Reading.date_time_format"),strtotime($reply->created_at)) }}</span></h4>
			...
			$.ajax({
				...
				success: function(r){
					...
					if(datas['success'] == true) {
						window.location.href	=	'{{ route("Library.librarydetail",["$topicData->id","$user_id"]) }}';
					}else {
