...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active">{{ trans("Edit Challenge") }} </li>
	</ol>
</section>
...
<div class="row pad"> 
	...
	{{ Form::open(['role' => 'form','url' =>  route("$modelName.update","$challengeDetails->id"),'class' => 'mws-form','files'=>'true','id'=>'challenges_form']) }}
	{{ Form::hidden('id',isset($challengeDetails->id) ? $challengeDetails->id :'',['class' => '','id'=>'user_id']) }}
	<div class="row pad">
			...
			<div id="" class="row col-md-12">	
				<div class="col-md-6">	
					<div class="form-group ">
						...
						<div class="mws-form-item">
							{{ Form::text('sponsor_name',$challengeDetails->sponsor_name,['class' => 'form-control']) }}
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
							{{ Form::text('challenge_name',$challengeDetails->challenge_name,['class' => 'form-control']) }}
					...
					<div class="form-group ">
						...
						<div class="mws-form-item">
							{{ Form::text('start_date',$challengeDetails->start_date,['class' => 'form-control start_date','readonly'=>'readonly']) }}
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
							{{ Form::text('end_date',$challengeDetails->end_date,['class' => 'form-control end_date','readonly'=>'readonly']) }}
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
						<div class="state_div">
							{{ Form::select('no_of_questions',array(null=>'Please select no of questions')+Config::get("no_of_questions"),$challengeDetails->no_of_questions,['class'=>'form-control state_id chosen-select']) }}
					...
					<div class="form-group">
						<div class="mws-form-row">
							...
							<div class="mws-form-item">
								{{ Form::textarea('term_condition',$challengeDetails->term_condition,['class' => 'form-control','id'=>'term_condition','rows'=>3]) }}
					...
					<div class="form-group image_section">
						<div class="mws-form-row">
							...
							<div class="mws-form-item">
								{{ Form::file('image','',['class' => 'form-control image11 ','type'=>'file']) }}
								...
								@if($challengeDetails->image != '' && File::exists(CHALLENGE_IMAGE_ROOT_PATH.$challengeDetails->image))
									<?php
										$image		=	CHALLENGE_IMAGE_URL.$challengeDetails->image;
									?>
				...
				<div class="col-md-6">
					<div class="form-group">
						...
						<div class="mws-form-item">
						<div class="state_div">
							{{ Form::select('grade_level',array(null=>'Please select grade')+Config::get("challenges_grade"),$challengeDetails->grade_level,['class'=>'form-control state_id chosen-select grade']) }}
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
							{{ Form::radio('assign_dancer',1,($challengeDetails->assign_dancer == 1) ? true :false,['class'=>'assign_dancer_range']) }}<label>All Dancer</label>
							{{ Form::radio('assign_dancer',0,($challengeDetails->assign_dancer == 0) ? true :false,['class'=>'assign_dancer_range']) }}<label>Select Age Range</label>
							...
							<div class="mws-form-item">
								<div class="col-md-6">
									<div class="form-group">
										...
										{{ Form::text('minimum_age',($challengeDetails->minimum_age>0)?$challengeDetails->minimum_age:'',['class'=>'form-control']) }}
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										{{ Form::text('maximum_age',($challengeDetails->maximum_age > 0)?$challengeDetails->maximum_age:'',['class'=>'form-control']) }}
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
							{{ Form::text('how_many_winners',$challengeDetails->how_many_winners,['class' => 'form-control']) }}
					...
					<div class="form-group">
						{!! HTML::decode( Form::label('laederboards',trans("Laederboards(Number of Performers)").'<span class="requireRed"> * </span>', ['class' => 'mws-form-label']))  !!}
						<div class="mws-form-item">
							{{ Form::text('laederboards',!empty($challengeDetails->laederboards)?$challengeDetails->laederboards:'10',['class' => 'form-control']) }}
						...
						<div class="mws-form-row">
							...
							<div class="mws-form-item">
								{{ Form::select('category[]',$challengesCategory,
									explode(",",$challengeDetails->category),
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
							{{ Form::textarea("description",$challengeDetails->description, ['class' => 'form-control textarea_resize','id' => 'description',"rows"=>3,"cols"=>3]) }}
					...
					<div class="form-group">
						...
						<div class="mws-form-item">
							{{ Form::textarea("instruction",$challengeDetails->instruction, ['class' => 'form-control textarea_resize','id' => 'instruction',"rows"=>3,"cols"=>3]) }}
							{{ Form::hidden("ck_instruction",'', ['class' => 'valid','id' => 'ck_instruction']) }}
				...
				<table class="table">
					<tbody id="prize_section">
						@if(!empty($prizeDetails ))
							@foreach($prizeDetails as $key => $prizeData)
							<?php //pr($options);die;?>
							<?php
								$prize_count			=	$prizeData->id;
							?>
							<tr class="" id="prize_contant_{{$prize_count}}" rel="{{$prize_count}}">
								{{ Form::hidden("prize[$prize_count][prize_id]",$prize_count,["class"=>"form-control"]) }}
								<td width="25%;">
									{{ Form::text("prize[$prize_count][prize_name]",$prizeData->prize_name,['class'=>"form-control","placeholder"=>"Prize Name"])}}
									<div class="error-message help-inline"></div>
								</td>
								<td width="30%;">
									{{ Form::textarea(
										 "prize[$prize_count][prize_description]",$prizeData->prize_description, ['class' => 'form-control',"placeholder"=>"Prize Description","cols"=>"3","rows"=>"4"]
										) 
									}}
									<div class="error-message help-inline"></div>
								</td>
								<td width="4%;">
									{{ Form::file("prize[$prize_count][image]",['class'=>"document_upload valid"])}}
									<a class="fancybox-buttons"  data-fancybox-group="button" href="<?php echo PRIZE_IMAGE_URL.$prizeData->image; ?>">
										<div class="usermgmt_image">
											<img class="img-circle" src="<?php echo WEBSITE_URL.'image.php?width=120px&height=120px&image='.PRIZE_IMAGE_URL.'/'.$prizeData->image ?>">
										</div>
									</a>
								</td>
								<td width="25%;">
									<a title="Delete" data-url="{{route($modelName.'.deletePrize')}}" onclick="delete_challenge_prize({{$prize_count}});" href="javascript:void(0);" class="btn btn-danger btn-small delete_challenge_prize_{{$prize_count}}"><span class="ti-trash"></span></a>
								</td>
							</tr>
							@endforeach
						@else
							<tr class="" id="prize_contant_{{$prize_count}}" rel="{{$prize_count}}">
								<td width="25%;">
									{{ Form::text("prize[$prize_count][prize_name]","",['class'=>"form-control","placeholder"=>"Prize Name"])}}
									<div class="error-message help-inline"></div>
								</td>
								<td width="30%;">
									{{ Form::textarea(
								...
								<td width="4%;">
									{{ Form::file("prize[$prize_count][image]",['class'=>"document_upload validate"."$prize_count"])}}
									<div class="error-message help-inline"></div>
								</td>
								<td width="25%;">
...
<script type="text/javascript">
	...
	function delete_challenge_prize(row_counter) {
		bootbox.confirm("Are you sure want to delete this ?",
		function(result){
			if(result){	 
				var URL	=	$(".delete_challenge_prize_"+row_counter).attr("data-url");
				$.ajax({
					headers: {
						'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
					},
					...
					success: function(r){
						...
						if(data['success'] == 1) {
							show_message("Challenge prize deleted successfully.",'success');
						}else{
							show_message("Challenge prize not deleted.",'error');
						}
					}
				});
			}
		...
		$.ajax({
			...
			success: function(r){
				...
				if(data['success'] == 1) {
					...
					show_message("Challenge update successfully.",'success');
				}
				else {
