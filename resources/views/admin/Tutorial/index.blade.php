...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"> {{ trans("Tutorials") }}</li>
	</ol>
</section>
		...
		{{ Form::hidden('display') }}
			<div class="col-md-3 col-sm-3">
				<div class="form-group ">  
					{{ Form::text('description',((isset($searchVariable['description'])) ? $searchVariable['description'] : ''), ['class' => 'form-control','placeholder'=>'Description']) }}
				</div>
			</div>
			<div class="col-md-3 col-sm-3">
			...
			<div class="col-md-3 col-sm-3 ">
				@if(Config::get('app.debug'))
					<a href='{{route("$modelName.add")}}'  class="btn btn-success btn-small align pull-right"> {{ trans("Add Tutorial") }} </a>
				@endif
			</div>
		{{ Form::close() }}
		...
		<div class="box-body">
			<table class="table table-hover">
				<thead>
					<tr>
						<th width="12%">
							<?php $pagenameimage = ($sortBy == 'youtube_url') ? ($sortBy == 'youtube_url' && $order == 'desc') ? '<img src="'.WEBSITE_IMG_URL.'down-arrow.png'.'" width="12px" height="12px">' : '<img src="'.WEBSITE_IMG_URL.'up-arrow.png'.'" width="12px" height="12px">' : '<img src="'.WEBSITE_IMG_URL.'up-down-arrow.png'.'" width="12px" height="12px">'; ?>
							{!!
								html_entity_decode(link_to_route(
									"$modelName.index",
									trans("Url").$pagenameimage,
									array(
									'sortBy' => 'youtube_url',
									'order' => ($sortBy == 'youtube_url' && $order == 'desc') ? 'asc' : 'desc',
									$query_string
									),
									array('class' => (($sortBy == 'youtube_url' && $order == 'desc') ? 'sorting desc' : (($sortBy == 'youtube_url' && $order == 'asc') ? 'sorting asc' : 'sorting')) )
								))
							!!}
						</th>
						<th width="20%">
							<?php $blocknameimage = ($sortBy == 'description') ? ($sortBy == 'description' && $order == 'desc') ? '<img src="'.WEBSITE_IMG_URL.'down-arrow.png'.'" width="12px" height="12px">' : '<img src="'.WEBSITE_IMG_URL.'up-arrow.png'.'" width="12px" height="12px">' : '<img src="'.WEBSITE_IMG_URL.'up-down-arrow.png'.'" width="12px" height="12px">'; ?>
							{!!
								html_entity_decode(link_to_route(
									"$modelName.index",
									trans("Description").$blocknameimage,
									array(
									'sortBy' => 'description',
						...
						<th width="10%">
							<?php $descriptionimage = ($sortBy == 'tutorial_order') ? ($sortBy == 'tutorial_order' && $order == 'desc') ? '<img src="'.WEBSITE_IMG_URL.'down-arrow.png'.'" width="12px" height="12px">' : '<img src="'.WEBSITE_IMG_URL.'up-arrow.png'.'" width="12px" height="12px">' : '<img src="'.WEBSITE_IMG_URL.'up-down-arrow.png'.'" width="12px" height="12px">'; ?>
							{!!
								html_entity_decode(link_to_route(
									"$modelName.index",
									trans("Order").$descriptionimage,
									array(
									'sortBy' => 'tutorial_order',
									'order' => ($sortBy == 'tutorial_order' && $order == 'desc') ? 'asc' : 'desc',
									$query_string
									),
									array('class' => (($sortBy == 'tutorial_order' && $order == 'desc') ? 'sorting desc' : (($sortBy == 'tutorial_order' && $order == 'asc') ? 'sorting asc' : 'sorting')) )
								))
							!!}
						</th>
						<th width="10%">
							...
							{!!
								html_entity_decode(link_to_route(
									"$modelName.index",
									trans("Created at").$createdatimage,
									array(
									'sortBy' => 'created_at',
								...
								<tr class="items-inner">
									...
									<td data-th='{{ trans("messages.$modelName.block_name") }}'>
										<a href="{{$result->youtube_url}}" target="_blank">
											{{ strip_tags($result->youtube_url) }}
										</a>
									</td>
									<td data-th='{{ trans("messages.$modelName.description") }}'>{{ str_limit(strip_tags($result->description),100) }}</td>
									<td data-th='{{ trans("Image") }}'>
										@if($result->image != '' && File::exists(TUTORIAL_IMAGE_ROOT_PATH.$result->image))
											<a class="fancybox-buttons" data-fancybox-group="button" href="<?php echo TUTORIAL_IMAGE_URL.$result->image; ?>">
												<div class="usermgmt_image">
													<img class="img-circle" src="<?php echo WEBSITE_URL.'image.php?width=75px&height=75px&cropratio=1:1&image='.TUTORIAL_IMAGE_URL.'/'.$result->image ?>">
												</div>
											</a>
										@else
									...
									<td  data-th='{{ trans("Order") }}'>
										<span style="color:#0088CC;cursor:pointer;" id="link_<?php echo $result->tutorial_order."_".$result->id ?>" onclick="change(this)">
											{{ $result->tutorial_order }}
										</span>
											<div id="change_div<?php echo $result->id ?>" style="display:none; ">
												...
												<a class="btn btn btn-success"  id="link_<?php echo $result->tutorial_order."_".$result->id ?>" onclick="order(this)"  href="javascript:void(0);">
 ...
 // for update the orderby value
  function order(obj){
	...
	$.ajax({
		...
		type: "POST",
		url: "<?php  echo route('Tutorial.changeOrder'); ?>",
